...
struct SwingAnalyzer {
    ...
    private struct Constants {
        ...
        // 頭の上下動（アドレス時の胴体長に対する割合）
        static let headMovementThresholdBad: Double = 0.20
        static let headMovementThresholdCheck: Double = 0.10
    }
    ...
    static func analyzeSwing(phases: SwingPhases, cache: [Int: [NormalizedLandmark]]) -> [AnalysisResult] {
        ...
        // 3. 頭の安定性診断（GitHub後発版のMVP 3項目評価を移植）
        results.append(evaluateHeadStability(address: addressLM, impact: impactLM))
    ...
    /// 頭の上下動を、アドレス時の胴体長に対する割合で評価する。
    private static func evaluateHeadStability(address: [NormalizedLandmark], impact: [NormalizedLandmark]) -> AnalysisResult {
        let torsoLength = sqrt(
            pow(Double(address[23].x - address[11].x), 2) +
            pow(Double(address[23].y - address[11].y), 2)
        )
        let movementRatio = abs(Double(impact[0].y - address[0].y)) / max(0.01, torsoLength)

        let evaluation: Evaluation
        let advice: String
        if movementRatio >= Constants.headMovementThresholdBad {
            evaluation = .bad
            advice = "頭の上下動が大きくなっています。目線の高さを保つ意識で再現性を高めましょう。"
        } else if movementRatio >= Constants.headMovementThresholdCheck {
            evaluation = .check
            advice = "頭が少し上下しています。無理に固定せず、軸の高さを揃える意識を持ちましょう。"
        } else {
            evaluation = .excellent
            advice = "頭の高さが安定しています。スイング軸を保てています。"
        }

        return AnalysisResult(
            title: "Head Stability",
            valueStr: String(format: "%.0f%% Move", movementRatio * 100),
